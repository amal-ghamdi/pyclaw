...
struct pdeParam
{
    ...
    real* dt_used;		// time step used in the completed computation (without reverting), used to increment global simulation time
    bool* revert;	        // boolean flag to determine whether the time
                                // step has to be reverted, this is because we
                                // cannot swap the buffer
                                // on the GPU itself, and it has to be done on
                                // the CPU.
                                // Alternatively, leave all the work of
                                // determining CFL violation and time step
                                // calculation to the CPU.
